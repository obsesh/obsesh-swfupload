...
package {
	...
	public class SWFUpload extends Sprite {
		...
		private var filePostType:String; //Obsesh photo type [look, avatar]
		...
		public function SWFUpload() {
			...
			this.filePostType = decodeURIComponent(root.loaderInfo.parameters.filePostType);
			...
			if (!this.filePostType) {
				this.filePostType = "look";
			}
			if (!this.fileTypes) {
				this.fileTypes = "*.*";
			}
		...
		*************************************************************** */
		...
		private function PrepareResizedImageCompleteHandler(event:ImageResizerEvent):void {
			...
			this.Debug("PrepareResizedImageCompleteHandler(): Finished resizing. Initializing MultipartURLLoader for file: " + this.filePostType + ".jpg");
			
			if (this.current_file_item != null) {
				// var extension:String = event.encoding == this.ENCODER_PNG ? ".png" : ".jpg"; (This is adding double extensions!)
				var extension:String = "";
				// this.current_file_item.resized_uploader = new MultipartURLLoader(event.data, this.current_file_item.file_reference.name + extension); (We've been getting way too many errors becuase of weird characters in the filename; Let's fixate this)
				this.current_file_item.resized_uploader = new MultipartURLLoader(event.data, this.filePostType + ".jpg" + extension);
				ExternalCall.UploadStart(this.uploadStart_Callback, this.current_file_item.ToJavaScriptObject());
			}
		}		
